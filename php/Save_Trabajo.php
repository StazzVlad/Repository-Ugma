<?php
    ...
    $asesor=limpiar_cadena($_POST['tutor']);
    $tutor=limpiar_cadena($_POST['tutor']);
...
/*
    ...
    */
    ...
    $filtros=[
        ...
        "$nota" => "[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ ]{1,50}",
        "$resumen" => "[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ ]{4,5000}",
        "$tutor" => "[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ ]{1,50}",
        "$asesor" => "[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ ]{1,50}"
    ];
/*
    ...
    */
    ...
    $check=conexion();
    $check=$check->query("SELECT tutor_nombre, tutor_id FROM tutor WHERE tutor_nombre = '$tutor'");
    if($check->rowCount()>0){
        $id = ($check->fetch(PDO::FETCH_ASSOC))['tutor_id'];
        echo $id;
    }
    else {
        echo '
